    ...
    if FORCE_SUB:
        try:
          ...
          if str(user.status) == "ChatMemberStatus.BANNED":
              await message.reply_text("You are Banned. Contact -- @Sunil_v")
              return 1
...
async def terms(client, message):
    ...
    buttons = InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("💬 Contact Now", url="http://t.me/SaveRestrictionContactBot")],
        ]
    )
...
async def plan(client, message):
    plan_text = (
        "> 💰 **Premium Price**:\n\n Starting from 50 INR accepted via **__Amazon Gift Card / Upi / Phonepe / G-pay__** (terms and conditions apply).\n"
